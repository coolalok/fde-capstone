...
**Affects:** `src/model_cache.py`, `src/config.py`, `src/classify.py`, `src/generate.py`, `src/guardrails.py`, `.env.example`, `tests/conftest.py`
...
**B. On-disk response cache, plus a larger retry budget.**
A cache keyed on the request; retries raised from 2 to 5 so the SDK's existing backoff gets a real chance to work.
